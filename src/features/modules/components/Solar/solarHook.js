...
// For webpack/create-react-app environments:
import companyLogo from '@assets/images/logo.png'; // Update path as needed
import schoolLogo from '@assets/images/tup_logo.png'; // Update path as needed
...
export const useSolarAnalytics = () => {
  ...
  const handleDownload = useCallback(async () => {
    try {
      ...
      // 1) HEADER BACKGROUND (Orange/Yellow fill for solar theme)
      const headerHeight = 30;
      doc.setFillColor(255, 180, 0); // Sunny yellow/orange for solar theme
      doc.rect(0, 0, pageWidth, headerHeight, 'F');
  
      // 2) ADD LOGOS - Using imported image files
      // Add company logo on the left
      doc.addImage(companyLogo, 'PNG', 15, 5, 20, 20);
      
      // Add school logo on the right
      // If it's an SVG, you could use 'SVG' as the format instead
      doc.addImage(schoolLogo, 'SVG', pageWidth - 35, 5, 20, 20);
      ...
      if (chartRef.current) {
        try {
          ...
          // Add title - centered
          doc.setFontSize(14);
          const chartTitleText = 'Solar Power Generation Chart';
      ...
      doc.setFontSize(14);
      const tableTitleText = 'Solar Power Generation Data Table';
      ...
      doc.autoTable({
        ...
        headStyles: { 
          fillColor: [255, 180, 0], // Match solar theme
          halign: 'center',
          fontSize: 11
        },
        columnStyles: {
        ...
        alternateRowStyles: {
          fillColor: [252, 248, 227] // Light yellow for alternate rows
        }
      });
      ...
      // Solar-themed header bar for RECOMMENDATIONS
      ...
      doc.text("Based on the projected increase in solar power generation, we recommend:", 20, yPosition);
      doc.setFont(undefined, 'normal');
      ...
      // Solar-specific sources
      doc.text("1. Placeholder source for solar irradiance data", 20, yPosition + 7);
